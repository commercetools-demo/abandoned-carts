...
import {
  createAzureServiceBusOrderCreatedSubscription,
  createGcpPubSubOrderCreatedSubscription,
} from './actions';
...
async function postDeploy(properties: Map<string, unknown>): Promise<void> {
  ...
  switch (connectProvider) {
    case 'AZURE': {
      ...
      await createAzureServiceBusOrderCreatedSubscription(
        apiRoot,
        connectionString
      );
      break;
    ...
    default: {
      ...
      await createGcpPubSubOrderCreatedSubscription(
        apiRoot,
        topicName,
        projectId
      );
