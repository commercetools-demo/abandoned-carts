 ...
 *
 * This file is COPIED into `mc-app/src/email-template.js`. The Merchant
 * Center application authors the template and previews it; this application
 * renders and sends it. They must agree exactly on the placeholder syntax
 * and on the substitution, or a merchandiser inserts something the sender
 * cannot fill and a shopper receives `{{cartTotal}}` in their inbox.
 *
 * Connect builds each application from its own folder with its own
 * package.json, so no import can cross between them and there is no package
 * worth publishing for one module. `tests/unit/contract-copies.spec.js`
